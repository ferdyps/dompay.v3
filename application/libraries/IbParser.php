...
class BCAParser
{
    ...
    function __construct( $conf )
    {
        ...
        //echo(date( 'Y|m|d|H|i|s', $this->conf['time'] ) );
...
function getTransactions()
    {
        ...
        foreach( $rows as $key => $val )
        {
            ...
            $rows[$key]['tanggal']  = substr( $rows[$key][0], -5 );
            if ( stristr( $rows[$key][0], 'pend' ) )
			$rows[$key][0] = $tgl;
            $detail         = explode( "<td valign='top'>", $rows[$key][1] );
            $rows[$key]['type']  = $detail[1];
            $rows[$key][1]  = explode( '<br>', $detail[0] );
            $rows[$key]['nominal']  = substr(trim($rows[$key][1][count($rows[$key][1])-1]),0,-3);
            $rows[$key]['nominal']  = str_replace(',','',$rows[$key]['nominal'] );
			unset( $rows[$key][1][count($rows[$key][1])-1] );
            foreach( $rows[$key][1] as $k => $v )
                $rows[$key][1][$k] = trim( strip_tags( $v ) );
            $rows[$key]['keterangan'] = implode( " ", $rows[$key][1] );
